...
import cc.wdev.webapp.es.domain.dto.CourseDto;
import cc.wdev.webapp.es.domain.dto.InstructorDto;
...
public class CoreTools {
    ...
    @Tool(name = "searchCourse", description = """
        搜索课程
        """)
    public List<CourseDto> searchCourse(@ToolParam(description = "关键字") String keyword) {
        ...
        return page.isEmpty() ? emptyList() : page.getContent().stream().map((entity) ->
            CourseDto.builder()
    ...
    @Tool(name = "searchInstructor", description = """
        搜索讲师
        """)
    public List<InstructorDto> searchInstructor(@ToolParam(description = "关键字") String keyword) {
        PageRequest request = PageRequest.builder().page(1).size(10).q(keyword).build();
